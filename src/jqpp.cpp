...
JQ::Value::Value(Value &&other) noexcept
    : Value()
{
    (*this) = std::move(other);
}

JQ::Value::Value(const Value &other) noexcept
    : Value()
{
    (*this) = other;
}

JQ::Value::Value(jv value) noexcept
    : Value()
{
    (*this) = value;
}
...
{
    static std::map<jv_kind, Type> Kind2Type{{JV_KIND_NUMBER, VT_NUMBER},
...
{
    if (prv<Value_P>().assertThat(JV_KIND_NUMBER) && jv_is_integer(prv<Value_P>().value_)) {
...
JQ::JQ::JQ()
    : HasPrivate<JQ_P>(new JQ_P(this))
{}

JQ::JQ::~JQ() {}

void JQ::JQ::setErrorCallback(MsgCallback_Ft Ft)
{
    prv<JQ_P>().ErrCb = Ft;
    if (prv<JQ_P>().ErrCb == nullptr) {
        ...
        jq_set_error_cb(
            prv<JQ_P>().jq_,
            [](void *data, jv msg) {
                reinterpret_cast<JQ *>(data)->prv<JQ_P>().ErrCb(Value(msg));
                jv_free(msg);
            },
            this);
    }
...
JQ::JQ::MsgCallback_Ft JQ::JQ::getErrorCallback()
{
    return prv<JQ_P>().ErrCb;
}

void JQ::JQ::setStderrCallback(MsgCallback_Ft Ft)
{
    ...
    if (prv<JQ_P>().StderrCb == nullptr) {
        ...
        jq_set_stderr_cb(
            prv<JQ_P>().jq_,
            [](void *data, jv msg) {
                reinterpret_cast<JQ *>(data)->prv<JQ_P>().StderrCb(Value(msg));
                jv_free(msg);
            },
            this);
    }
...
JQ::JQ::MsgCallback_Ft JQ::JQ::getStderrCallback()
{
    return prv<JQ_P>().StderrCb;
}

void JQ::JQ::setDebugCallback(MsgCallback_Ft Ft)
{
    prv<JQ_P>().DebugCb = Ft;
    if (prv<JQ_P>().DebugCb == nullptr) {
        ...
        jq_set_debug_cb(
            prv<JQ_P>().jq_,
            [](void *data, jv msg) {
                reinterpret_cast<JQ *>(data)->prv<JQ_P>().DebugCb(Value(msg));
                jv_free(msg);
            },
            this);
    }
...
void JQ::JQ::setAttributes(const Value &v)
{
    jq_set_attrs(prv<JQ_P>().jq_, v.copy());
}

JQ::Value JQ::JQ::getAttributes()
{
    return Value(jq_get_attrs(prv<JQ_P>().jq_));
}

void JQ::JQ::setAttribute(const Value &Attr, const Value &Val)
{
    jq_set_attr(prv<JQ_P>().jq_, Attr.copy(), Val.copy());
}

JQ::Value JQ::JQ::getAttribute(const Value &Attr)
{
    return Value(jq_get_attr(prv<JQ_P>().jq_, Attr.copy()));
}

JQ::Value JQ::JQ::getLibDirs()
{
    return Value(jq_get_lib_dirs(prv<JQ_P>().jq_));
}

JQ::Value JQ::JQ::getJqOrigin()
{
    return Value(jq_get_jq_origin(prv<JQ_P>().jq_));
}

JQ::Value JQ::JQ::getProgOrigin()
{
    return Value(jq_get_prog_origin(prv<JQ_P>().jq_));
}

JQ::JQ::MsgCallback_Ft JQ::JQ::getDebugCallback()
{
...
void JQ::JQ::trace(bool On, bool WithDetail)
{
    prv<JQ_P>().runFlags = ((On) ? ((WithDetail) ? JQ_DEBUG_TRACE_ALL : JQ_DEBUG_TRACE) : 0);
}
...
JQ::Array JQ::JQ::run(const Value &input)
{
    ...
    jq_start(prv<JQ_P>().jq_, jv_copy(input.get()), prv<JQ_P>().runFlags );
...
{
    *(Value *)(this) = jv_array_concat(copy(), v.copy());
    return *this;
}
...
{
    *(Value *)(this) = jv_array_concat(copy(), jv_copy(v));
    return *this;
}

JQ::Array &JQ::Array::push(const Value &v)
{
    *(Value *)(this) = jv_array_append(copy(), v.copy());
    return *this;
}
...
{
    *(Value *)(this) = jv_array_append(copy(), jv_copy(v));
    return *this;
}

JQ::Array &JQ::Array::set(size_t Idx, const Value &v)
{
    *(Value *)(this) = jv_array_set(copy(), ArraySize_t(Idx), v.copy());
    return *this;
}
...
{
    *(Value *)(this) = jv_array_set(copy(), ArraySize_t(Idx), jv_copy(v));
    return *this;
}
